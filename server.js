...
app.get('/api/acft/person', (req, res) => {
...
app.get('/api/acft/test', (req, res) => {
    ...
    if (req.query.personID && req.query.latest) {
        ...
        pool.query(`SELECT person.id, person.name, person.gender, person.age, acft.id as acft_id, acft.age as acft_age, acft.mdl, acft.spt, acft.hrp, acft.sdc, acft.plk, acft.run, acft.walk, acft.bike, acft.swim, acft.kmrow, acft.date, acft.person_id FROM person LEFT JOIN acft ON person.id = acft.person_id WHERE person.id = $1 ORDER BY acft.date DESC LIMIT 1`, [id], (err, result) => {
            if (err) {
                console.error(err);
    ...
    /* All tests for person by ID */
        ...
        pool.query(`SELECT person.id, person.name, person.gender, person.age, acft.id as acft_id, acft.age as acft_age, acft.mdl, acft.spt, acft.hrp, acft.sdc, acft.plk, acft.run, acft.walk, acft.bike, acft.swim, acft.kmrow, acft.date, acft.person_id FROM person LEFT JOIN acft ON person.id = acft.person_id WHERE person.id = $1 ORDER BY acft.date DESC`, [id], (err, result) => {
            if (err) {
                console.error(err);
                res.status(500).send('Error reading person-acft join table');
            } else if (result.rows.length === 0) {
...
/* Create a person */
app.post('/api/acft/person', (req, res) => {
    const { name, gender, age } = req.body;
    pool.query(`INSERT INTO person (name, gender, age) VALUES ($1, $2, $3) RETURNING *`, [name, gender, age], (err, result) => {
        if (err) {
            console.error(err);
...
app.put('/api/acft/person/:id', (req, res) => {
    const id = req.params.id;
    const { name, gender, age } = req.body;
    pool.query('UPDATE person SET name=$1, gender=$2, age=$3 WHERE id=$4 RETURNING *', [name, gender, age, id], (err, result) => {
        if (err) {
            console.error(err);
...
app.patch('/api/acft/person/:id', (req, res) => {
    const id = req.params.id;
    const data = req.body;
...
/* Delete person by id */
app.delete('/api/acft/person/:id', (req, res) => {
    const id = req.params.id;
    pool.query('DELETE FROM person WHERE id = $1 RETURNING *', [id], (err, result) => {
        if (err) {
            console.error(err);
...
/* Create test */
app.post('/api/acft/test', (req, res) => {
    const { age, mdl, spt, hrp, sdc, plk, run, date, person_id } = req.body;
    pool.query("INSERT INTO acft (age, mdl, spt, hrp, sdc, plk, run, date, person_id) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *", [age, mdl, spt, hrp, sdc, plk, run, date, person_id], (err, result) => {
        if (err) {
            console.error(err);
...
app.put('/api/acft/test/:id', (req, res) => {
    const id = req.params.id;
    const { age, mdl, spt, hrp, sdc, plk, run, date, person_id } = req.body;
    pool.query('UPDATE acft SET age=$1, mdl=$2, spt=$3, hrp=$4, sdc=$5, plk=$6, run=$7, date=$8, person_id=$9 WHERE id=$10 RETURNING *', [age, mdl, spt, hrp, sdc, plk, run, date, person_id, id], (err, result) => {
        if (err) {
            console.error(err);
...
app.patch('/api/acft/test/:id', (req, res) => {
    const id = req.params.id;
    const data = req.body;
...
/* Delete test by ID */
app.delete('/api/acft/test/:id', (req, res) => {
    const id = req.params.id;
    pool.query('DELETE FROM acft WHERE id = $1 RETURNING *', [id], (err, result) => {
        if (err) {
            console.error(err);
